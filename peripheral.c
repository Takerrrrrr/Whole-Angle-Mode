...
/**** AXI-GPIO设备 ****/
#if defined(USE_HARDWARE_SPEC_SINGLE_GYRO_100MHZ)
// 陀螺频率字初始值
#define G1_freword_init 1000 * 1099511627776 / 100000000 // G1
...
XGpio G12_CHx_DATA_CHAN; // AXI_GPIO_1	// AXI_GPIO_1	高压DAC GPIO接口数据和通道初始值

// G1  GPIO接口结构作用
XGpio G1_freword;                    // AXI_GPIO_1  	G1频率字
XGpio G1_amp_i_modulation;           // AXI_GPIO_2 	   G1同相驱动力调幅系数(sin)
XGpio G1_amp_q_modulation;           // AXI_GPIO_20 	G1正交驱动力调幅系数(cos)
XGpio G1_cha_drive_phase_modulation; // AXI_GPIO_21 	G1 A通道驱动调相
XGpio G1_chb_drive_phase_modulation; // AXI_GPIO_22 	G1 B通道驱动调相
XGpio G1_cha_demod_phase_modulation; // AXI_GPIO_6  	G1 A通道解调调相
...
{
    // Description: GPIO初始化
    // axi_gpio0 高压DAC数据和通道号
    XGpio_Initialize(&G12_CHx_DATA_CHAN, XPAR_AXI_GPIO_0_DEVICE_ID);
    XGpio_SetDataDirection(&G12_CHx_DATA_CHAN, 1, 0x0000); // Data_CHx
    XGpio_SetDataDirection(&G12_CHx_DATA_CHAN, 2, 0x0000); // Channel_CHx
    for (u8 DCDACInitCount = 0; DCDACInitCount < 8; DCDACInitCount++)
    {
        XGpio_DiscreteWrite(&G12_CHx_DATA_CHAN, 1, 32768);
        XGpio_DiscreteWrite(&G12_CHx_DATA_CHAN, 2, (DCDACInitCount + 1));
        usleep(1000);
        XGpio_DiscreteWrite(&G12_CHx_DATA_CHAN, 2, 0x0000);
    }

    // Gyr1
    // axi_gpio1 G1_freword
    XGpio_Initialize(&G1_freword, XPAR_AXI_GPIO_1_DEVICE_ID);
    ...
    // axi_gpio2 G1_amp_i_modulation
    XGpio_Initialize(&G1_amp_i_modulation, XPAR_AXI_GPIO_2_DEVICE_ID);
    ...
    // axi_gpio20 G1_amp_q_modulation
    XGpio_Initialize(&G1_amp_q_modulation, XPAR_AXI_GPIO_20_DEVICE_ID);
    ...
    // axi_gpio21 G1_cha_drive_phase_modulation
    XGpio_Initialize(&G1_cha_drive_phase_modulation, XPAR_AXI_GPIO_21_DEVICE_ID);
    ...
    // axi_gpio22 G1_chb_drive_phase_modulation
    XGpio_Initialize(&G1_chb_drive_phase_modulation, XPAR_AXI_GPIO_22_DEVICE_ID);
    ...
    // axi_gpio16 G1_cha_demod_phase_modulation
    XGpio_Initialize(&G1_cha_demod_phase_modulation, XPAR_AXI_GPIO_6_DEVICE_ID);
    ...
    // axi_gpio11 G1_chb_demod_phase_modulation
    XGpio_Initialize(&G1_chb_demod_phase_modulation, XPAR_AXI_GPIO_11_DEVICE_ID);
    ...
    // axi_gpio7 G1_demod_amp
    XGpio_Initialize(&G1_demod_amp, XPAR_AXI_GPIO_7_DEVICE_ID);
    XGpio_SetDataDirection(&G1_demod_amp, 1, 0xFFFFFFFF);
    XGpio_SetDataDirection(&G1_demod_amp, 2, 0xFFFFFFFF);

    // axi_gpio8 G1_demod_phase
    XGpio_Initialize(&G1_demod_phase, XPAR_AXI_GPIO_8_DEVICE_ID);
    XGpio_SetDataDirection(&G1_demod_phase, 1, 0xFFFFFFFF);
    XGpio_SetDataDirection(&G1_demod_phase, 2, 0xFFFFFFFF);

    // axi_gpio16 G1_ADC_data
    XGpio_Initialize(&G1_ADC_data, XPAR_AXI_GPIO_16_DEVICE_ID);
    XGpio_SetDataDirection(&G1_ADC_data, 1, 0xFFFF);
    XGpio_SetDataDirection(&G1_ADC_data, 2, 0xFFFF);

    // axi_gpio17 G1_NCO
    XGpio_Initialize(&G1_NCO, XPAR_AXI_GPIO_17_DEVICE_ID);
...
{
    // Description: 写入高压数据和高压通道号
    // AXIGPIO通道1 -> Data_CHx: 高压数值
    // AXIGPIO通道2 -> Channel_CHx: 使用的通道号
    XGpio_DiscreteWrite(&G12_CHx_DATA_CHAN, 1, Data_CHx); // Data_CHx
    usleep(1);
    XGpio_DiscreteWrite(&G12_CHx_DATA_CHAN, 2, Channel_CHx); // Channel_CHx
    usleep(9);
    XGpio_DiscreteWrite(&G12_CHx_DATA_CHAN, 2, 0x0000);
    usleep(9);
}
...
/**** AXI-GPIO设备 ****/
#elif defined(USE_HARDWARE_SPEC_SINGLE_GYRO_HV)
// 陀螺频率字初始值
...
void axi_gpio_init(void)
{
    ...
    XGpio_SetDataDirection(&G1_NCO, 1, 0x00000000);
    XGpio_SetDataDirection(&G1_NCO, 2, 0x00000000);
    XGpio_DiscreteWrite(&EC_RS, 1, 0x0000);
    XGpio_DiscreteWrite(&EC_RS, 2, 0x0000);
    
    // axi_gpio15 EC_Angel
    XGpio_Initialize(&EC_Angel, XPAR_AXI_GPIO_15_DEVICE_ID);
    XGpio_SetDataDirection(&G1_NCO, 1, 0xFFFFFFFF);
}
...
#elif defined(USE_HARDWARE_SPEC_DUAL_GYRO_STD)
// 陀螺频率字初始值
#define G1_freword_init 1000 * 4294967295 / 5000000 // G1
#define G2_freword_init 1000 * 4294967295 / 5000000 // G2
// 各个同相正交调幅系数，驱动解调调相值初始值
#define G1_cha_sin_drive_phase_init 0x0000          // 0-0000 30-2000 45-2D41 60-376C 90-4000
#define G1_cha_cos_drive_phase_init 0x4000          // 0-4000 30-376C 45-2D41 60-2000 90-0000
#define G1_chb_sin_drive_phase_init 0x0000          // 0-0000 30-2000 45-2D41 60-376C 90-4000
...
#define G1_chb_sin_demod_phase_init 0x0000          // 0-0000 30-2000 45-2D41 60-376C 90-4000
#define G1_chb_cos_demod_phase_init 0x4000          // 0-4000 30-376C 45-2D41 60-2000 90-0000
#define G2_cha_sin_drive_phase_init 0x0000          // 0-0000 30-2000 45-2D41 60-376C 90-4000
#define G2_cha_cos_drive_phase_init 0x4000          // 0-4000 30-376C 45-2D41 60-2000 90-0000
#define G2_chb_sin_drive_phase_init 0x0000          // 0-0000 30-2000 45-2D41 60-376C 90-4000
#define G2_chb_cos_drive_phase_init 0x4000          // 0-4000 30-376C 45-2D41 60-2000 90-0000
#define G2_cha_sin_demod_phase_init 0x0000          // 0-0000 30-2000 45-2D41 60-376C 90-4000
#define G2_cha_cos_demod_phase_init 0x4000          // 0-4000 30-376C 45-2D41 60-2000 90-0000
#define G2_chb_sin_demod_phase_init 0x0000          // 0-0000 30-2000 45-2D41 60-376C 90-4000
#define G2_chb_cos_demod_phase_init 0x4000          // 0-4000 30-376C 45-2D41 60-2000 90-0000
// 陀螺同相幅值初始化
#define G1_cha_amp_i_init 0x0000
#define G1_chb_amp_i_init 0x0000
#define G2_cha_amp_i_init 0x0000
#define G2_chb_amp_i_init 0x0000
// 陀螺正交幅值初始化
#define G1_cha_amp_q_init 0x0000
#define G1_chb_amp_q_init 0x0000
#define G2_cha_amp_q_init 0x0000
#define G2_chb_amp_q_init 0x0000

XGpio G12_CHx_DATA_CHAN; // AXI_GPIO_1	高压DAC GPIO接口数据和通道初始值

// G1  GPIO接口结构作用
XGpio G1_freword;                    // AXI_GPIO_0  	G1频率字
XGpio G1_amp_i_modulation;           // AXI_GPIO_8 	G1同相驱动力调幅系数(sin)
XGpio G1_amp_q_modulation;           // AXI_GPIO_9 	G1正交驱动力调幅系数(cos)
XGpio G1_cha_drive_phase_modulation; // AXI_GPIO_13 	G1 A通道驱动调相
XGpio G1_chb_drive_phase_modulation; // AXI_GPIO_12 	G1 B通道驱动调相
...
// G2  GPIO接口结构作用
XGpio G2_freword;                    // AXI_GPIO_14	G2频率字
XGpio G2_amp_i_modulation;           // AXI_GPIO_19 	G2同相驱动力调幅系数(sin)
XGpio G2_amp_q_modulation;           // AXI_GPIO_20	G2正交驱动力调幅系数(cos)
XGpio G2_cha_drive_phase_modulation; // AXI_GPIO_15	G2 A通道驱动调相
XGpio G2_chb_drive_phase_modulation; // AXI_GPIO_16	G2 B通道驱动调相
XGpio G2_cha_demod_phase_modulation; // AXI_GPIO_17	G2 A通道解调调相
XGpio G2_chb_demod_phase_modulation; // AXI_GPIO_18	G2 B通道解调调相

XGpio G2_cha_demod_IQ; // AXI_GPIO_23	G2 A通道解调后IQ值
XGpio G2_chb_demod_IQ; // AXI_GPIO_24	G2 B通道解调后IQ值
XGpio G2_demod_amp;    // AXI_GPIO_21	G2 解调幅值
XGpio G2_demod_phase;  // AXI_GPIO_22	G2 解调相位

void axi_gpio_init(void)
{
    // Description: GPIO初始化
    // axi_gpio1 高压DAC数据和通道号
    XGpio_Initialize(&G12_CHx_DATA_CHAN, XPAR_AXI_GPIO_1_DEVICE_ID);
    XGpio_SetDataDirection(&G12_CHx_DATA_CHAN, 1, 0x0000); // Data_CHx
    XGpio_SetDataDirection(&G12_CHx_DATA_CHAN, 2, 0x0000); // Channel_CHx
    for (u8 DCDACInitCount = 0; DCDACInitCount < 8; DCDACInitCount++)
    {
    ...
    // Gyr2
    // axi_gpio14 G2_freword output data
    XGpio_Initialize(&G2_freword, XPAR_AXI_GPIO_14_DEVICE_ID);
    XGpio_SetDataDirection(&G2_freword, 1, 0x00000000);
    XGpio_SetDataDirection(&G2_freword, 2, 0x00000000);

    XGpio_DiscreteWrite(&G2_freword, 1, G2_freword_init);

    // axi_gpio19 G2_amp_i_modulation
    XGpio_Initialize(&G2_amp_i_modulation, XPAR_AXI_GPIO_19_DEVICE_ID);
    XGpio_SetDataDirection(&G2_amp_i_modulation, 1, 0x0000);
    XGpio_SetDataDirection(&G2_amp_i_modulation, 2, 0x0000);
    XGpio_DiscreteWrite(&G2_amp_i_modulation, 2, G2_chb_amp_i_init);
    XGpio_DiscreteWrite(&G2_amp_i_modulation, 1, G2_cha_amp_i_init);

    // axi_gpio20 G2_amp_q_modulation
    XGpio_Initialize(&G2_amp_q_modulation, XPAR_AXI_GPIO_20_DEVICE_ID);
    XGpio_SetDataDirection(&G2_amp_q_modulation, 1, 0x0000);
    XGpio_SetDataDirection(&G2_amp_q_modulation, 2, 0x0000);
    XGpio_DiscreteWrite(&G2_amp_q_modulation, 2, G2_chb_amp_q_init);
    XGpio_DiscreteWrite(&G2_amp_q_modulation, 1, G2_cha_amp_q_init);

    // axi_gpio15 G2_cha_drive_phase_modulation
    XGpio_Initialize(&G2_cha_drive_phase_modulation, XPAR_AXI_GPIO_15_DEVICE_ID);
    XGpio_SetDataDirection(&G2_cha_drive_phase_modulation, 1, 0x0000);
    XGpio_SetDataDirection(&G2_cha_drive_phase_modulation, 2, 0x0000);

    XGpio_DiscreteWrite(&G2_cha_drive_phase_modulation, 2, G2_cha_sin_drive_phase_init);
    XGpio_DiscreteWrite(&G2_cha_drive_phase_modulation, 1, G2_cha_cos_drive_phase_init);

    // axi_gpio16 G2_chb_drive_phase_modulation
    XGpio_Initialize(&G2_chb_drive_phase_modulation, XPAR_AXI_GPIO_16_DEVICE_ID);
    XGpio_SetDataDirection(&G2_chb_drive_phase_modulation, 1, 0x0000);
    XGpio_SetDataDirection(&G2_chb_drive_phase_modulation, 2, 0x0000);

    XGpio_DiscreteWrite(&G2_chb_drive_phase_modulation, 2, G2_chb_sin_drive_phase_init);
    XGpio_DiscreteWrite(&G2_chb_drive_phase_modulation, 1, G2_chb_cos_drive_phase_init);

    // axi_gpio17 G2_cha_demod_phase_modulation
    XGpio_Initialize(&G2_cha_demod_phase_modulation, XPAR_AXI_GPIO_17_DEVICE_ID);
    XGpio_SetDataDirection(&G2_cha_demod_phase_modulation, 1, 0x0000);
    XGpio_SetDataDirection(&G2_cha_demod_phase_modulation, 2, 0x0000);
    XGpio_DiscreteWrite(&G2_cha_demod_phase_modulation, 2, G2_cha_sin_demod_phase_init);
    XGpio_DiscreteWrite(&G2_cha_demod_phase_modulation, 1, G2_cha_cos_demod_phase_init);

    // axi_gpio18 G2_chb_demod_phase_modulation
    XGpio_Initialize(&G2_chb_demod_phase_modulation, XPAR_AXI_GPIO_18_DEVICE_ID);
    XGpio_SetDataDirection(&G2_chb_demod_phase_modulation, 1, 0x0000);
    XGpio_SetDataDirection(&G2_chb_demod_phase_modulation, 2, 0x0000);
    XGpio_DiscreteWrite(&G2_chb_demod_phase_modulation, 2, G2_chb_sin_demod_phase_init);
    XGpio_DiscreteWrite(&G2_chb_demod_phase_modulation, 1, G2_chb_cos_demod_phase_init);

    // axi_gpio23 G2_cha_demod_IQ
    XGpio_Initialize(&G2_cha_demod_IQ, XPAR_AXI_GPIO_23_DEVICE_ID);
    XGpio_SetDataDirection(&G2_cha_demod_IQ, 1, 0xFFFFFFFF);
    XGpio_SetDataDirection(&G2_cha_demod_IQ, 2, 0xFFFFFFFF);

    // axi_gpio24 G2_chb_demod_IQ
    XGpio_Initialize(&G2_chb_demod_IQ, XPAR_AXI_GPIO_24_DEVICE_ID);
    XGpio_SetDataDirection(&G2_chb_demod_IQ, 1, 0xFFFFFFFF);
    XGpio_SetDataDirection(&G2_chb_demod_IQ, 2, 0xFFFFFFFF);

    // axi_gpio21 G2_demod_amp
    XGpio_Initialize(&G2_demod_amp, XPAR_AXI_GPIO_21_DEVICE_ID);
    XGpio_SetDataDirection(&G2_demod_amp, 1, 0xFFFFFFFF);
    XGpio_SetDataDirection(&G2_demod_amp, 2, 0xFFFFFFFF);

    // axi_gpio22 G2_demod_phase
    XGpio_Initialize(&G2_demod_phase, XPAR_AXI_GPIO_22_DEVICE_ID);
    XGpio_SetDataDirection(&G2_demod_phase, 1, 0xFFFFFFFF);
    XGpio_SetDataDirection(&G2_demod_phase, 2, 0xFFFFFFFF);
}
...
void G2_write_freword(u32 freword)
{
    // Description: 写入2号陀螺的频率字的值
    // AXIGPIO通道1 -> u32 freword: 频率字
    XGpio_DiscreteWrite(&G2_freword, 1, freword);
    usleep(1);
}
...
void G2_write_amp_i_modulation(u8 channel, short amp_i_modulation)
{
    // Description: 写入2号陀螺，同相驱动力调幅系数的值
    // u8 channel: 陀螺通道号，1为A通道，2为B通道
    // short amp_i_modulation：同相驱动力调幅系数的值
    XGpio_DiscreteWrite(&G2_amp_i_modulation, channel, amp_i_modulation);
    usleep(1);
}
...
void G2_write_amp_q_modulation(u8 channel, short amp_q_modulation)
{
    // Description: 写入2号陀螺，正交驱动力调幅系数的值
    // u8 channel: 陀螺通道号，1为A通道，2为B通道
    // short amp_q_modulation：正交驱动力调幅系数的值
    XGpio_DiscreteWrite(&G2_amp_q_modulation, channel, amp_q_modulation);
    usleep(1);
}
...
void G2_write_drive_phase_modulation(u8 channel, short sin, short cos)
{
    // Description: 写入2号陀螺，调整对应驱动调相器的相位
    // u8 channel: 陀螺通道号，1为A通道，2为B通道
    // short sin:相位对应sin值，定点数格式：1位符号+1位整数+14位小数
    // short cos:相位对应cos值，定点数格式：1位符号+1位整数+14位小数
    if (channel == 1)
    {
        XGpio_DiscreteWrite(&G2_cha_drive_phase_modulation, 1, cos); // cos
        usleep(1);
        XGpio_DiscreteWrite(&G2_cha_drive_phase_modulation, 2, sin); // sin
        usleep(1);
    }
    else if (channel == 2)
    {
        XGpio_DiscreteWrite(&G2_chb_drive_phase_modulation, 1, cos); // cos
        usleep(1);
        XGpio_DiscreteWrite(&G2_chb_drive_phase_modulation, 2, sin); // sin
        usleep(1);
    }
    else
    {
...
void G2_write_demod_phase_modulation(u8 channel, short sin, short cos)
{
    // Description: 写入2号陀螺，调整对应解调调相器的相位
    // u8 channel: 陀螺通道号，1为A通道，2为B通道
    // short sin:相位对应sin值，定点数格式：1位符号+1位整数+14位小数
    // short cos:相位对应cos值，定点数格式：1位符号+1位整数+14位小数
    if (channel == 1)
    {
        XGpio_DiscreteWrite(&G2_cha_demod_phase_modulation, 1, cos); // cos
        usleep(1);
        XGpio_DiscreteWrite(&G2_cha_demod_phase_modulation, 2, sin); // sin
        usleep(1);
    }
    else if (channel == 2)
    {
        XGpio_DiscreteWrite(&G2_chb_demod_phase_modulation, 1, cos); // cos
        usleep(1);
        XGpio_DiscreteWrite(&G2_chb_demod_phase_modulation, 2, sin); // sin
        usleep(1);
    }
    else
    {
...
s32 G2_read_demod_IQ(u8 channel, u32 Gyro_IQ)
{
    // Description: 读取2号陀螺解调后的IQ值
    // u8 channel: 陀螺通道号，1为A通道，2为B通道
    // u32 Gyro_IQ: 陀螺IQ选择，1为读取Q，2为读取I
    // return: s32 G2解调后IQ值
    s32 temp = 0;
    if (channel == 1)
    {
        temp = XGpio_DiscreteRead(&G2_cha_demod_IQ, Gyro_IQ);
    }
    else if (channel == 2)
    {
        temp = XGpio_DiscreteRead(&G2_chb_demod_IQ, Gyro_IQ);
    }
    else
    {
    }
...
u32 G2_read_demod_amp(u8 channel)
{
    // Description: 读取2号陀螺解调后的幅值
    // u8 channel: 陀螺通道号，1为A通道，2为B通道
    // return: s32 G2解调后的幅值
    s32 temp = 0;
    temp = XGpio_DiscreteRead(&G2_demod_amp, channel);
    return temp;
}
...
s32 G2_read_demod_phase(u8 channel)
{
    // Description: 读取2号陀螺解调后的相位
    // u8 channel: 陀螺通道号，1为A通道，2为B通道
    // return: s32 G2解调后的相位
    s32 temp = 0;
    temp = XGpio_DiscreteRead(&G2_demod_phase, channel);
    return temp;
}
